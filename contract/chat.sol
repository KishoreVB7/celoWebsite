...
contract Chat {
    //The address of the user who is waiting to be matched
    ...
    //The mapping used for pairing one user with another
    mapping(address => address) internal addresses;

    //Mapping used to store messages
    mapping(address => mapping(uint256 => Message)) internal messages;

    //Mapping used to store the value one sent another
    mapping(address => uint256) internal messageAmounts;

    //Mapping used to store the user last match with
    mapping( address => address) internal previousMatch;

    //Modifier that is used to check if the caller has a partner
    modifier hasPartner(){
        require(isAddressAssigned(),"You are not matched with an address");
        _;
    }

    //Event that is emmited when user is connected
    event connected(address indexed Person1, address indexed person2);

    //Event that is emmited when there is a fund transfer between the users
    event transferBetweenUsers(address indexed sender, address indexed reciever, uint value);

    /* checks if there's one waiting,
    if not, it makes the sender address the address that is waiting */
    function assignAddress() public {
        /* checks if the waiting address is empty (address(0) means that if the address is empty) */
        if (waitingAddress == address(0)) {
            waitingAddress = msg.sender;
        /* If the above statement is not true, it means that someone is already waiting, 
            The below statement checks that if the caller is waiting, if not then he is matched with 
            the waiting address
        */
        ...
        emit connected(msg.sender, waitingAddress);
    }
    ...
    function writeMessage(string memory _text) public hasPartner{
    ...
    //Connect again with the last disconnected person
    function connectWithPreviousMatch() public{
        require(previousMatch[msg.sender] == waitingAddress);
        addresses[msg.sender] = waitingAddress;
        addresses[waitingAddress] = msg.sender;
        waitingAddress = address(0);
    }
    ...
    function transferFunds(uint256 _amount) public payable hasPartner {
        require(
        ...
        emit transferBetweenUsers(msg.sender, addresses[msg.sender], _amount);
    }
    ...
    function removeMatch() public {
        require(addresses[msg.sender] != address(0), "You don't have a match");
        previousMatch[msg.sender] = addresses[msg.sender];
        previousMatch[addresses[msg.sender]] = msg.sender;
